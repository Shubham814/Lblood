...
class New extends Component {
  ...
  makeRequest = (location, message, cause, blood_group) => {
    ...
    if (location && message && cause && blood_group) {
      db.ref('requests/' + Math.random().toString(36).slice(2)).set({
        name: name,
      ...
      Alert.alert(
        'Error',
        'Sorry! we cannot record your request. Try again in a moment.'
      );
    }
  };
  async loadFonts() {
    ...
    if (!this.state.fontsLoaded) {
      ...
      return (
        <SafeAreaView style={styles.container}>
          ...
          <Text style={styles.action}>Choose Blood Group</Text>
          <Picker
          ...
          <TextInput
            placeholder="Message (I need blood immediately for my son.)"
            placeholderTextColor="rgba(0,0,0,0.6)"
            caretHidden={false}
            style={styles.input}
            onChangeText={(val) => {
          ...
          <Text style={styles.action}>Cause of Requesting</Text>
          <TextInput
            placeholder="Why are you Requesting(Accident,Thalassemia,etc.)"
            placeholderTextColor="rgba(0,0,0,0.6)"
            caretHidden={false}
            style={styles.input}
            onChangeText={(val) => {
